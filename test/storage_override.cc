...
TEST(base_case) {
  ...
  cache<int, int> c = cache<int, int>::builder()
      .not_thread_safe()
      .template storage_override<std::tuple<int, std::shared_ptr<int>>>(
          [](const std::shared_ptr<int>& ptr) -> std::tuple<int, const std::shared_ptr<int>&> {
            return { 17, ptr };
          },
          [](const std::tuple<int, std::shared_ptr<int>>& tpl) -> const std::shared_ptr<int>& {
